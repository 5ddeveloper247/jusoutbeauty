...
//     		$arrRes['totalOrderAmount'] = number_format($this->getOrderTotalAmount($row->ORDER_ID),2);
    ...
    public function getAllSearchOrderData($flag='',$orderNumber='',$orderStatus='',$shippmentStatus='',$startDate='',$endDate=''){
        // dd('hello');
    	if($flag == 1){
    	...
    	if($orderNumber != ''){
    		$where = array_merge($where, array(['a.ORDER_ID','like',"%$orderNumber%"]));
            // dd($where);
    		// $where = array_merge($where, array(['fut.LAST_NAME','like',"%$orderNumber%"]));
    	}
    	if($orderStatus != ''){
...
//     	exit();
        ...
        // dd($where);
    	$result = DB::table('jb_order_tbl as a')->select('a.*','jopt.PAYMENT_STATUS', 'a.ORDER_ID')
    	->join ( 'jb_order_payment_tbl as jopt', 'a.ORDER_ID', '=', 'jopt.ORDER_ID' )
    	->join ( 'fnd_user_tbl as fut', 'a.USER_ID', '=', 'fut.USER_ID' )
