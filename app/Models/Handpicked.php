...
{
     use HasFactory;
     protected $table="jb_product_handpicked_tbl";

     protected $fillable = [
        'HANDPICKED_ID',
        'USER_ID',
        'HANDPICKEDPRODUCT_ID',
        'PRODUCT_ID',
     ];
    ...
    public function deleteHandpicked($product,$product_id){
         ...
         DB::beginTransaction();
             try{
                foreach ($product as $p){
                    $product=DB::table('jb_product_handpicked_tbl')->where('PRODUCT_ID',$product_id )->delete();
                 }
                 DB::commit();
                     $status=true;
                     $message="success";
     ...
     public function insertHandpicked($product,$userId,$product_id){
          ...
          try{
            $i=0;
            foreach ($product as $p){
                $result = DB::table ( 'jb_product_handpicked_tbl' )->insertGetId (
                    array ( 'USER_ID' => $userId,
                            'HANDPICKEDPRODUCT_ID' => isset($p['id']) ? $p['id'] : $p[$i],
