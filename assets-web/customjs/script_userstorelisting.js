...
//	}
    ...
    $scope.loadMoreProductsForNutrition = function() {
        $scope.productsToShowForNutrition += 4;
    };
		...
		$http({
			...
			if($scope.productType == 'bundle'){
				...
				if(product_details != null && product_details != ''){
					...
					$scope.QuickView_invQty = product_details['INV_QUANTITY'];
...
//		});
	...
	$scope.getAllUserStoreListingLov = function(){
		...
		$http({
			data: temp+"&"+$scope.tokenHash,
			url : site+'/getAllUserStoreListingLov',
			method: "POST",
			async: false,
			headers: {'Content-Type': 'application/x-www-form-urlencoded'}
	...
	$scope.getAllUserStoreListingLov();
	...
	$scope.filter = function(){
		...
		$http({
			data: temp+"&"+$scope.tokenHash,
			url : site+'/getUserSearchStoreListing',
			method: "POST",
			async: false,
			headers: {'Content-Type': 'application/x-www-form-urlencoded'}
